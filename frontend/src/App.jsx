      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        <form onSubmit={(e) => { e.preventDefault(); login(email, contraseña); }} className="space-y-4">
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="usuario@email.com" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Contraseña</label>
            <div className="relative">
              <input type={mostrar ? 'text' : 'password'} value={contraseña} onChange={(e) => setContraseña(e.target.value)} placeholder="••••••••" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required />
              <button type="button" onClick={() => setMostrar(!mostrar)} className="absolute right-3 top-2.5 text-gray-500">
            ...
            {transacciones.map(tx => (
              <div key={tx._id} className="bg-white bg-opacity-10 rounded-xl p-4 backdrop-blur">
                <div className="flex justify-between">
                  <div>
                    <p className="font-semibold">{tx.emisor._id === usuario.id ? 'Enviado a' : 'Recibido de'}</p>
                    <p className="text-blue-200 text-sm">{tx.emisor._id === usuario.id ? tx.receptor.nombre : tx.emisor.nombre}</p>
                  </div>
                  <p className={`font-bold ${tx.emisor._id === usuario.id ? 'text-red-300' : 'text-green-300'}`}>
                    {tx.emisor._id === usuario.id ? '-' : '+'} ${tx.monto.toFixed(2)}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="bg-white text-gray-900 rounded-t-3xl w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={realizar} disabled={loading} className="flex-1 py-2 bg-blue-600 text-white rounded-lg disabled:bg-gray-400">
      ...
      {showNuevo && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white text-gray-900 rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Nombre" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              <input type="password" value={contraseña} onChange={(e) => setContraseña(e.target.value)} placeholder="Contraseña" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              <input type="number" value={saldo} onChange={(e) => setSaldo(e.target.value)} placeholder="Saldo inicial" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
              <div className="flex gap-3">
                <button onClick={() => setShowNuevo(false)} className="flex-1 py-2 border border-gray-300 rounded-lg">Cancelar</button>
                <button onClick={crear} disabled={loading} className="flex-1 py-2 bg-blue-600 text-white rounded-lg disabled:bg-gray-400">
